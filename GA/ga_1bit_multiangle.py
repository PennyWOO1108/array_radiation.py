...
def get_fitness(pop): # 计算种群中各个个体的适应度
    ...
    for i in range(0,POP_SIZE):
        result.append(radiation(data_x,data_y,n_data,x[:,i:i+1]))
...
def read_csv():
    ...
    data_y = []  # 归一化
    csv_data = pd.read_csv("../dataset/newdata.csv") # 同文件夹下的data文件，读取单个阵元数据
    ...
    for i in range(0, n_data):  # 0循环到n_data-1，共n_data次
        ...
        data_y.append(csv_data.loc[i][1])  # 归一
...
def radiation(data_x, data_y, n_data, phase):
    ...
    for i in range(0, n_data):  # 生成数据长度和hfss得到的csv文件长度保持一致（才能做对比）
        ...
        k_d = k * (np.sin(data_x[i] * np.pi / 180) - np.sin(theta0 * np.pi / 180))
    ...
    # performance.append(data_array[theta0 + 180]-value)  #主瓣值-副瓣值
    # print(data_array[theta0+90])
    return data_array[theta0 + 90]-value
